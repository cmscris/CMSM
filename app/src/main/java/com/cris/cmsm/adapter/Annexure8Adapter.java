...
public class Annexure8Adapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    private static final int TYPE_HEADER = 0;
    private static final int TYPE_ITEM = 1;
    private static final int TYPE_FOOTER = 2;
    List<Annexure8vo> list;
    Context context;
    ReportHeaderView reportHeaderView;
    private String date;

    public Annexure8Adapter(Context context, ReportHeaderView reportHeaderView, List<Annexure8vo> list) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof VHItem) {
            int pos = position - 1;
            Annexure8vo model = getItem(pos);
            if (model != null) {
                ((VHItem) holder).tv_rly_div.setText(model.getRlycode());
                ((VHItem) holder).tv_seb.setText(model.getSebs().equalsIgnoreCase("~") ? "TOTAL" : model.getSebs());
                ...
                if (pos > 0) {
                    if (model.getRlycode().equals("~") && model.getSebs().equals("~")) {
    ...
    private Annexure8vo getItem(int position) {
