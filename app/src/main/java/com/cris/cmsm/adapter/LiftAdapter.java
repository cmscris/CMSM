...
public class LiftAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    List<LiftAssestVO> list;
    Context context;
    ReportHeaderView reportHeaderView;
    private String date;

    public LiftAdapter(Context context, ReportHeaderView reportHeaderView, List<LiftAssestVO> list) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == TYPE_ITEM) {
            View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.lift_inflater, parent, false);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof VHItem) {
            int pos = position - 1;
            final LiftAssestVO model = getItem(pos);
            VHItem item = (VHItem) holder;
            item.tv_location_type.setText(model.getLocationType());
    ...
    private LiftAssestVO getItem(int position) {
        return list.get(position);
    }

    class VHItem extends RecyclerView.ViewHolder {
        TextView tv_railway, tv_division, tv_location_type, tv_location, tv_make, tv_year_installation, tv_maintenance;
