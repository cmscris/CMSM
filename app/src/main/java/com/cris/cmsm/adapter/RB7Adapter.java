...
public class RB7Adapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    private static final int TYPE_HEADER = 0;
    private static final int TYPE_ITEM = 1;
    private static final int TYPE_FOOTER = 2;
    List<AnnexureRB7vo> list;
    Context context;
    ReportHeaderView reportHeaderView;
    private String date;

    public RB7Adapter(Context context, ReportHeaderView reportHeaderView, List<AnnexureRB7vo> list) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == TYPE_ITEM) {
            View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.rb_mis_list_seven, parent, false);
            return new VHItem(view);
        } else if (viewType == TYPE_HEADER) {
            View headerView = LayoutInflater.from(parent.getContext()).inflate(R.layout.rb1_header_seven, parent, false);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof VHItem) {
            int pos = position - 1;
            AnnexureRB7vo model = getItem(pos);
            if (model != null) {
                try {
                    ...
                    if (pos > 0) {
                        if (model.getRlycode().equalsIgnoreCase("Z") && model.getDiv().equalsIgnoreCase("Z")) {
                            ((VHItem) holder).tv_div.setText("");
                            ((VHItem) holder).tv_rly.setText("GRAND TOTAL");
                        } else if (getItem(pos - 1).getRlycode().equalsIgnoreCase(model.getRlycode())) {
                            ((VHItem) holder).tv_rly.setText("");
                        } else {
                            ((VHItem) holder).tv_rly.setText(model.getRlycode());
                        }

                        if (getItem(pos - 1).getDiv().equalsIgnoreCase(model.getDiv())) {
                            ((VHItem) holder).tv_div.setText("");
                        } else {
                            ((VHItem) holder).tv_div.setText(model.getDiv().equalsIgnoreCase("Z") ? "TOTAL" : model.getDiv());
                        }

                    } else {
                        if (model.getRlycode().equalsIgnoreCase("Z") && model.getRlycode().equalsIgnoreCase("Z")) {
                            ((VHItem) holder).tv_div.setText("");
                            ((VHItem) holder).tv_rly.setText("GRAND TOTAL");
                        } else {
                            ((VHItem) holder).tv_rly.setText(model.getRlycode());
                    ...
                    if (pos == 0) {
                        ((VHItem) holder).tv_div.setTypeface(null, Typeface.BOLD);
                        ((VHItem) holder).tv_rly.setTypeface(null, Typeface.BOLD);
                        ...
                        ((VHItem) holder).tv_div.setTypeface(null);
                        ((VHItem) holder).tv_rly.setTypeface(null);
    ...
    private AnnexureRB7vo getItem(int position) {
        return list.get(position);
    }

    class VHItem extends RecyclerView.ViewHolder {
        TextView tv_rly, tv_div, tv_traction, tv_non_traction, tv_consumed_total, tv_bill_traction, tv_bill_non_traction, tv_bill_total, tv_avg_traction, tv_avg_nontraction, tv_avg_total;
