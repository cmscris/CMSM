...
public class RB6Adapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    private static final int TYPE_HEADER = 0;
    private static final int TYPE_ITEM = 1;
    private static final int TYPE_FOOTER = 2;
    List<AnnexureRB6vo> list;
    Context context;
    ReportHeaderView reportHeaderView;
    private String date;

    public RB6Adapter(Context context, ReportHeaderView reportHeaderView, List<AnnexureRB6vo> list) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof VHItem) {
            int pos = position - 1;
            AnnexureRB6vo model = getItem(pos);
            if (model != null) {
                try {
            ...
            if (list != null && list.size() > 0) {
                AnnexureRB6vo footer = list.get(list.size() - 1);
                if (footer != null) {
    ...
    private AnnexureRB6vo getItem(int position) {
