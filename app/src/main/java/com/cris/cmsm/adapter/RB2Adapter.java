...
public class RB2Adapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    private static final int TYPE_HEADER = 0;
    private static final int TYPE_ITEM = 1;
    private static final int TYPE_FOOTER = 2;
    List<AnnexureRB2vo> list;
    Context context;
    ReportHeaderView reportHeaderView;
    private String date;

    public RB2Adapter(Context context, ReportHeaderView reportHeaderView, List<AnnexureRB2vo> list) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof VHItem) {
            int pos = position - 1;
            AnnexureRB2vo model = getItem(pos);
            if (model != null) {
                try {
                    ((VHItem) holder).tv_rly_div.setText(model.getRlycode());
                    ((VHItem) holder).tv_traction.setText(model.getPconsumption());
                    ((VHItem) holder).tv_non_traction.setText(model.getPbilling());
                    ((VHItem) holder).tv_consumed_total.setText(model.getPaverage());
                    ((VHItem) holder).tv_bill_traction.setText(model.getConsumption());
                    ((VHItem) holder).tv_bill_non_traction.setText(model.getBilling());
                    ((VHItem) holder).tv_bill_total.setText(model.getAverage());
                    ((VHItem) holder).tv_avg_traction.setText(model.getInccon());
                    ((VHItem) holder).tv_avg_nontraction.setText(model.getInccost());
                    ((VHItem) holder).tv_avg_total.setText(model.getIncrate());
    ...
    private AnnexureRB2vo getItem(int position) {
