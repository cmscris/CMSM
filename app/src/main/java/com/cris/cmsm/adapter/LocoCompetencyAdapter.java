...
public class LocoCompetencyAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    private static final int TYPE_HEADER = 0;
    private static final int TYPE_ITEM = 1;
    private static final int TYPE_FOOTER = 2;
    private Context context;
    private List<LocoCompetencyVO> list;
    private OnItemClickListener listener;
    private ReportHeaderView reportHeaderView;
    private RequestPresenter requestPresenter;


    public LocoCompetencyAdapter(Context context, ReportHeaderView reportHeaderView, OnItemClickListener listener, List<LocoCompetencyVO> list) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof VHItem) {
            ...
            int pos = position - 1;
            final LocoCompetencyVO model = getItem(pos);


            item.tv_code.setText(model.getType());
            item.tv_done_date.setText(model.getLast_drive_date());
    ...
    public void onModelClick(LocoCompetencyVO model) {
    ...
    private LocoCompetencyVO getItem(int position) {
