...
public class Annexure7Adapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    private static final int TYPE_HEADER = 0;
    private static final int TYPE_ITEM = 1;
    private static final int TYPE_FOOTER = 2;
    List<Annexure7vo> list;
    Context context;
    ReportHeaderView reportHeaderView;
    private String date;

    public Annexure7Adapter(Context context, ReportHeaderView reportHeaderView, List<Annexure7vo> list) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == TYPE_ITEM) {
            View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.annexure_three, parent, false);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof VHItem) {
            ...
            if (model != null) {
                ...
                if (model.getSebs().equalsIgnoreCase("~"))
                    ((VHItem) holder).tv_rly_div.setText("TOTAL");
                else
                    ((VHItem) holder).tv_rly_div.setText(model.getSebs());
            }
    ...
    private Annexure7vo getItem(int position) {
