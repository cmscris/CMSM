...
public class Annexure10_1Adapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public Annexure10_1Adapter(Context context, ReportHeaderView reportHeaderView, List<Annexure10vo> list) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == TYPE_ITEM) {
            View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.annexure10_list_item1, parent, false);
            return new VHItem(view);
        } else if (viewType == TYPE_HEADER) {
            View headerView = LayoutInflater.from(parent.getContext()).inflate(R.layout.annexure_header_view, parent, false);
        ...
        if (holder instanceof VHItem) {
            ...
            if (model.getRlycode() != null && model.getRlycode().equalsIgnoreCase("Z")) {
                try {
                    ...
                    ((VHItem) holder).tv_traction.setText(model.getPconsumption());
                    ((VHItem) holder).tv_non_traction.setText(model.getConsumption());
                    ((VHItem) holder).tv_consumed_total.setText(model.getPbilling());
                    ((VHItem) holder).tv_bill_traction.setText(model.getBilling());
                    ((VHItem) holder).tv_bill_non_traction.setText(model.getPaverage());
                    ((VHItem) holder).tv_bill_total.setText(model.getAverage());
                    ((VHItem) holder).tv_avg_traction.setText(model.getPpercentage());
                    ((VHItem) holder).tv_avg_nontraction.setText(model.getPercentage());
                } catch (Exception e) {
                    e.printStackTrace();
                }
            } else {
