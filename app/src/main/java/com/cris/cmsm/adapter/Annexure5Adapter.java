...
public class Annexure5Adapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    private static final int TYPE_HEADER = 0;
    private static final int TYPE_ITEM = 1;
    private static final int TYPE_FOOTER = 2;
    List<AnnexureReport> list;
    Context context;
    ReportHeaderView reportHeaderView;
    private String date;

    public Annexure5Adapter(Context context, ReportHeaderView reportHeaderView, List<AnnexureReport> list) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == TYPE_ITEM) {
            View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.annexure_mis_list_item, parent, false);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof VHItem) {
            int pos = position - 1;
            AnnexureReport model = getItem(pos);
            ...
            if (pos > 0) {
                ...
                } else if (getItem(pos - 1).getRailway().equalsIgnoreCase(model.getRailway())) {
    ...
    private AnnexureReport getItem(int position) {
