...
public class Annexure9Adapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    private static final int TYPE_HEADER = 0;
    private static final int TYPE_ITEM = 1;
    private static final int TYPE_FOOTER = 2;
    List<Annexure9vo> list;
    Context context;
    ReportHeaderView reportHeaderView;
    private String date;

    public Annexure9Adapter(Context context, ReportHeaderView reportHeaderView, List<Annexure9vo> list) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof VHItem) {
            int pos = position - 1;
            Annexure9vo model = getItem(pos);
            if (model != null) {
                ((VHItem) holder).tv_rly_div.setText(model.getState());
                ((VHItem) holder).tv_seb.setText(model.getRlycode().equalsIgnoreCase("~") ? "TOTAL" : model.getRlycode());
                ((VHItem) holder).tv_traction.setText(model.getConsumption());
                ((VHItem) holder).tv_non_traction.setText(model.getBilling());
                ((VHItem) holder).tv_consumed_total.setText(model.getExcessSMD());
                ((VHItem) holder).tv_bill_traction.setText(model.getLowpf());
                ((VHItem) holder).tv_bill_non_traction.setText(model.getDelayedPayment());
                ((VHItem) holder).tv_bill_total.setText(model.getExcessConsumption());
                ((VHItem) holder).tv_avg_traction.setText(model.getExcessFCA());
                ((VHItem) holder).tv_avg_nontraction.setText(model.getAverage());
                ((VHItem) holder).tv_avg_total.setText(model.getPercentage());

                if (pos > 0) {
                    if (model.getState().equals("~") && model.getRlycode().equals("~")) {
                        ((VHItem) holder).tv_rly_div.setText("");
                        ((VHItem) holder).tv_seb.setText("GRAND TOTAL");
                    } else if (getItem(pos - 1).getState().equalsIgnoreCase(model.getState())) {
                        ((VHItem) holder).tv_rly_div.setText("");
                    } else {
                        ((VHItem) holder).tv_rly_div.setText(model.getState());
                    }
                } else {
                    if (model.getState().equals("~") && model.getState().equals("~")) {
                        ((VHItem) holder).tv_rly_div.setText("");
                        ((VHItem) holder).tv_seb.setText("GRAND TOTAL");
                    } else {
                        ((VHItem) holder).tv_rly_div.setText(model.getState());
    ...
    private Annexure9vo getItem(int position) {
