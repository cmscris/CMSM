...
public class OutGoingAdpater extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    private static final int TYPE_ITEM = 1;
    private static final int TYPE_HEADER = 0;
    private static final int TYPE_FOOTER = 2;
    List<OutGoingFeederVo> list;
    Context context;
    RequestSSAssets requestSSAssets;

    public OutGoingAdpater(Context context, RequestSSAssets requestSSAssets, List<OutGoingFeederVo> list) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof VHItem) {
            int pos = position - 1;
            OutGoingFeederVo model = getItem(pos);
            ...
            if (model != null) {
                ...
                if (requestSSAssets.getAssetsName().equalsIgnoreCase("LT PANEL")) {
                    ...
                    if (model.getRelayCount() == -100) {
                        item.tv_meter_available.setText("Feeder wise Relay details");
                    } else
    ...
    private OutGoingFeederVo getItem(int position) {
