...
public class JitinRLSlidingLayout extends ViewGroup {
	private static final String TAG = "JitinPaneLayout";
	private boolean isOpenDirection = true;

	public static final boolean RIGHT_DRAWER = false;
	public static final boolean LEFT_DRAWER = true;
	/**
	 ...
	 */
	private static final int DEFAULT_FADE_COLOR = 0x888888;
	...
	public JitinRLSlidingLayout(Context context) {
		this(context, null);
	}

	public JitinRLSlidingLayout(Context context, AttributeSet attrs) {
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		final int width = r - l;
		final int paddingStart = isLayoutRtl ? getPaddingRight()
				: getPaddingLeft();
		final int paddingEnd = isLayoutRtl ? getPaddingLeft()
				: getPaddingRight();
		...
		for (int i = 0; i < childCount; i++) {
			...
			if (lp.slideable) {
				...
				final int range = Math.min(nextXStart, width - paddingEnd
						...
						- xStart - margin;
				mSlideRange = range;
				final int lpMargin = isLayoutRtl ? lp.rightMargin
						: lp.leftMargin;
	...
	private void onPanelDragged(int newLeft) {
		...
		final int newStart = isLayoutRtl ? getWidth() - newLeft - childWidth
				: newLeft;
	...
	protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
		...
		if (mCanSlide && !lp.slideable && mSlideableView != null) {
			...
			if (isLayoutRtlSupport()) {
				mTmpRect.left = Math.max(mTmpRect.left,
						mSlideableView.getRight());
			} else {
				mTmpRect.right = Math.min(mTmpRect.right,
						mSlideableView.getLeft());
			}
		...
		public int clampViewPositionHorizontal(View child, int left, int dx) {
			...
			if (isLayoutRtlSupport()) {
				int startBound = getWidth()
						- (getPaddingRight() + lp.rightMargin + mSlideableView
								.getWidth());
	...
	interface SlidingPanelLayoutImpl {
		void invalidateChildRegion(JitinRLSlidingLayout parent, View child);
	}

	static class SlidingPanelLayoutImplBase implements SlidingPanelLayoutImpl {
		public void invalidateChildRegion(JitinRLSlidingLayout parent,
				View child) {
	...
	class AccessibilityDelegate extends AccessibilityDelegateCompat {
		...
		public void onInitializeAccessibilityNodeInfo(View host,
			...
			info.setClassName(JitinRLSlidingLayout.class.getName());
			info.setSource(host);
		...
		public void onInitializeAccessibilityEvent(View host,
			...
			event.setClassName(JitinRLSlidingLayout.class.getName());
		}
	...
	private class DisableLayerRunnable implements Runnable {
		...
		@Override
		public void run() {
			if (mChildView.getParent() == JitinRLSlidingLayout.this) {
	...
	public void setOpenDirection(boolean isOpenDirection) {
		this.isOpenDirection = isOpenDirection;
	}
